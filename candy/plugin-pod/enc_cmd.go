...
// loadPodDeployConfig fetches the per-host DeployConfig plugin-side via the cycle-free
// loaderkit.LoadHostDeployConfigViaExecutor (#55 coneC Unit C2 — this retired the former
// deploykit.LoadDeployConfigViaSeam host-handler round-trip;
// loaderkit already imports deploykit so the helper lives there) rather than letting
// EncPlanFor/EncStatus reach the placement-dependent bare deploykit.LoadDeployConfig()
...
// *DeployConfig — the new helper drives the full LoadUnified pipeline over the reverse channel
...
var loadPodDeployConfig = func() (*deploykit.DeployConfig, error) {
